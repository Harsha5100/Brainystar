...
public class BsUserService {
    ...
    public User userValidate(BsUserPojo bsUserPojo) {
        User user = bsUserRepository.findByUserNameAndAndPasswordUser(
...
//                }
    ...
    public void downloadFeeDueReportExcel(OutputStream out, ReportPojo reportPojo) {
        try {
            ...
            headerRow1.createCell(2).setCellValue("Grade");
            ...
            for (StudentFeePojo list : studentDueList) {
                if (list.getDueAmount() > 0) {
                    ...
                    row.createCell(2).setCellValue(list.getGradeMaster().getGradeName());
...
//            headerRow1.createCell(39).setCellValue("Total Paid");
    ...
    public void downloadFeeCollectedReportExcel(OutputStream out, ReportPojo reportPojo) {
        try {
            ...
            for (FeeReceiptPojo list : feeReceiptList) {
                ...
                row.createCell(2).setCellValue(student.getGradeMaster().getGradeName());
    ...
    public PdfPTable createFirstTableFeeDueReport(ReportPojo reportPojo) {
        ...
        PdfPCell pc4 = new PdfPCell(new Phrase("Grade", font));
        ...
        for (StudentFeePojo list : studentDueList) {
            if (list.getDueAmount() > 0) {
                ...
                table.addCell(new Phrase(list.getGradeMaster().getGradeName() + "", font1));
...
//        table5.addCell(new Phrase("", ""));
    ...
    public PdfPTable createFirstTableFeeCollectedReport(ReportPojo reportPojo) {
        ...
        for (FeeReceiptPojo list : feePojoList) {
            ...
            Student student=bsStudentRepository.findByStudentName(list.getStudentName());
            table.addCell(new Phrase(student.getGradeMaster().getGradeName() + "", font1));
    ...
    public FeeReceiptPojo getAllPrintReceipt(Long id) {
        ...
        FeeReceiptPojo feeReceiptPojo = new FeeReceiptPojo();
        feeReceiptPojo.setStudentPojo(studentPojo);
        feeReceiptPojo.setTotalPaid(studentFee.getPaidAmount());
        feeReceiptPojo.setCardAmt(studentFee.getCardAmt());
        feeReceiptPojo.setCashAmt(studentFee.getCashAmt());
        feeReceiptPojo.setStudentFee(studentFee);
        List<FeeReceipt> receiptList=bsFeeReceiptRepository.findByStudentFee(studentFee);
